...
def build_temporal_cost(q_token_idx, Lq, Lk, device, dtype):
    ...
    raw_seq_len = q_token_idx[0].get('raw_seq_len')

    query_idx = torch.arange(Lq, device=device, dtype=torch.long)
    query_frames = query_idx // tokens_per_frame
    valid_q = None
    if raw_seq_len is not None:
        valid_q = query_idx < int(raw_seq_len)
