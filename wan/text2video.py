...
from .utils.prompt_relay import maybe_prepare_prompt_relay
...
class WanT2V:
    ...
    def _prepare_prompts(self, input_prompt, frame_num, size, prompt_filepath):
        latent_frames = (frame_num - 1) // self.vae_stride[0] + 1
        width, height = size
        h_lat = int(height) // self.vae_stride[1]
        w_lat = int(width) // self.vae_stride[2]
        q_token_idx, full_prompt = maybe_prepare_prompt_relay(
        ...
        if q_token_idx is not None:
            normalized_q_token_idx = []
            for seg in q_token_idx:
                normalized_q_token_idx.append({
                    "window": seg["window"],
                    "sigma": torch.tensor(seg["sigma"], dtype=self.param_dtype),
                    "midpoint": seg["midpoint"],
                    "tokens_per_frame": seg["tokens_per_frame"],
                    "raw_seq_len": seg["raw_seq_len"],
                    "local_token_idx": torch.tensor(seg["local_token_idx"], dtype=torch.long),
                })
            q_token_idx = normalized_q_token_idx
    ...
    def generate(self,
        ...
        Args:
            ...
            prompt_filepath (`str`, *optional*, defaults to None):
                Path to a Prompt Relay JSON file containing `global_prompt`, `local_prompts`,
                and optional `segment_lengths` for temporal prompt control.
